...
class FCTS(loader.Module):
    ...
    async def client_ready(self):
    	if not self._client.hikka_me.premium:
            raise loader.LoadError("This module is for Telegram Premium only!") # Немного взял кода из модуля TgStatus от @hikarimods
    	self.default = {
    ...
    @loader.command(ru_doc="[имя статуса] - поставить этот статус | .statuslist для просмотра ваших установленных статусов")
